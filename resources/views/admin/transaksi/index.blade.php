                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle" id="transaksiTable">
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach ($groupedTransactions as $index => $group)
                                @php
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge bg-dark">{{ $firstTransaction->order_id }}</span></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <i class="fas fa-user-circle fa-lg me-2 text-secondary"></i>
                                            {{ $firstTransaction->user->nama ?? '-' }}
                                        </div>
                                    </td>
                                    <td>{!! $items !!}</td>
                                    <td>
                                        <span class="badge bg-success px-3 py-2">
                                            ...
                                            Rp {{ number_format($group->sum('total_harga'), 0, ',', '.') }}
                                        </span>
                                    </td>
                                    <td>
                                        @if ($firstTransaction->event_date)
                                            <span class="badge bg-info px-3 py-2">
                                                <i class="fas fa-calendar-day me-1"></i>
                                                {{ \Carbon\Carbon::parse($firstTransaction->event_date)->format('d M Y') }}
                                            </span>
                                        @else
                                    ...
                                    <td>
                                        <span class="badge bg-secondary px-3 py-2 venue-badge"
                                            ...
                                            onclick="showMap('{{ $firstTransaction->event_place }}')"
                                            data-location="{{ $firstTransaction->event_place }}">
                                            <i class="fas fa-map-marker-alt me-1"></i>
                                            {{ $firstTransaction->event_place ?? '-' }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge bg-primary px-3 py-2">
                                            ...
                                            @endphp
                                            {{ $paymentMethods[$firstTransaction['payment_method']] ?? ($firstTransaction['payment_method'] ?? '-') }}
                                        </span>
                                    </td>
                                    <td>
                                        @if ($firstTransaction->status_pemesanan == 'Selesai')
                                            <span class="badge bg-success px-3 py-2">
                                        ...
                                        <div class="d-flex gap-2 justify-content-center">
                                            @if ($firstTransaction->status_pemesanan != 'Selesai')
                                                <button class="btn btn-sm btn-outline-success update-status"
                                                    data-order-id="{{ $firstTransaction->order_id }}">
                                                    <i class="fas fa-clock me-1"></i>
                                                    Proses
                                                </button>
                                            @else
            ...
            $('.update-status').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '{{ route('transaksi.update-status') }}',
                            type: 'POST',
                            data: {
                                order_id: orderId,
                                _token: '{{ csrf_token() }}'
